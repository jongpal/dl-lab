...
def get_accuracy(input, target):
    return torch.sum(input == target) / len(input)
...
def action_to_id(a):
    ...
    # elif all(a == [0.0, 0.1, 0.0]): return ACCELERATE        # ACCELERATE: 3
    # elif all(a == [0.0, 0.0, 0.1]): return BRAKE             # BRAKE: 4
    if all(np.isclose(a, [-1.0, 0.0, 0.0])): return LEFT               # LEFT: 1
    elif all(np.isclose(a, [1.0, 0.0, 0.0])): return RIGHT             # RIGHT: 2
    elif all(np.isclose(a, [0.0, 1, 0.0])): return ACCELERATE        # ACCELERATE: 3
    elif all(np.isclose(a, [0.0, 0.0, np.float32(0.2)])): return BRAKE             # BRAKE: 4
    # elif all(np.isclose(a, [0.0, 0.1, 0.0])): return ACCELERATE        # ACCELERATE: 3
    # elif all(np.isclose(a, [0.0, 0.0, 0.1])): return BRAKE             # BRAKE: 4
    else:       
